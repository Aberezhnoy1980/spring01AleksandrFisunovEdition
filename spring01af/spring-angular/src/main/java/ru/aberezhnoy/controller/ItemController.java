...
import ru.aberezhnoy.service.ItemServiceImpl;
...
//    }
    ...
    @GetMapping("/items")
    public List<ItemDto> findAll(@RequestParam(name = "minPrice", required = false) Integer minPrice, @RequestParam(name = "maxPrice", required = false) Integer maxPrice) {
...
//        Category category = categoryService.findByTitle(dto.getCategoryTitle()).get();
        item.setCategory(categoryService.findByTitle(dto.getCategory()).get());
